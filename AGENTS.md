...
- Go：`go build ./...`。代码生成:`uv run python scripts/gen_proto.py`(all.pb → internal/pb)、
  ...
  `uv run python scripts/gen_icons.py`(UI 图标 → img/{filter,blood,static,worldmap,flower,medal,egg,glass}:属性/
  六维/搭档标记、血脉主图标、手挑杂项、手挑大地图 POI、稀兽花种的属性花图、奖牌小图、精灵蛋图与蛋品类角标;图集精灵从解包属性 JSON + 图集 PNG 裁切,
  奖牌与精灵蛋等整张贴图直接转码;glass 是炫彩色卡的素材(遮罩/粒子层/隐藏炫彩整卡与标记图,见 docs/data.md);
  webp 保持原始解包文件名,语义键→原名索引写入 names.json;
