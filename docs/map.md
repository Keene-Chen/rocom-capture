...
POI 与眠枭之星收集、野生宠物图层、实体下发范围、涂地。索引本身由 rocom-parse 的 `gen_gamedata.py`
与 `gen_bigmap.py` 产出到 `internal/gamedata/data/`(`make gamedata`),生成管线见其 docs/data.md;
游戏消息见 [协议说明](protocol.md)。
...
**不咕钟零件**(收集品,道具 104501「能打开某个神秘守护地的大门」)与星星同为
...
**点数**(启用行,卡洛西亚大陆 10003):魔力之源 43(另魔法学院 10018 有 4)、炼金釜 24(另家园种植园 30002 有 1)、
...
28→27,`got>=tot` 判定作为防御保留);蓝独立星 98 里也有 4 颗不计区域(计入 94)。故**不要拿配置点数当分母**算进度,
...
画出游戏内那张**色卡**(`DB.Glass` → `GlassCard`,画法见 rocom-parse docs/data.md 的炫彩色卡段)。
...
## 8. 稀兽花种(实时地图页「稀兽花种」图层)

大世界同时开着二十来朵**稀兽花种**,每朵关着一只混血精灵,打赢可花体力捕捉;其中三朵是活动
限定的**命定花种**(7 星,`spec_flower_seed_id`/`activity_id` 非空,普通花种为 5 星)。
本图层把它们画在地图上,并标出里面那只精灵**是不是异色或炫彩**。

### 8.1 异色/炫彩随花种列表下发

三条消息,`internal/scene/flower.go` 各解一条:

| opcode | 消息 | 给什么 | 何时发 |
| --- | --- | --- | --- |
| `0x0375` | `ZoneSceneQueryBossNpcInfoRsp` | 花种**全集**:位置/里面是哪只精灵/星级/有效期/**异色与炫彩** | 客户端登录后自动请求一次;玩家开花种面板再请求 |
| `0x0376` | `ZoneSceneSpecFlowerSeedInfoNty` | **变动的那几朵**(`flowers` 是与列表同构的 `BossNpcInfos`) | 服务器主动推,不用请求 |
| `0x0338` | `ZoneSceneTeamBattleInfoQueryRsp` | 某一朵的炫彩(兜底,见下) | 玩家在地图上点中某朵花(`query_source=1`)、或走到花前(=2) |

变异就在列表项 `BossNpcInfos.BossNpcInfo` 里:`mutation_type`(#9,位标志,bit0 异色
`MDT_SHINING`、bit3 炫彩 `MDT_GLASS`)+ `glass_info`(#25,说明是哪一种炫彩)。判据照客户端
`UMG_ChallengeItem_C:RefreshFlowerHeadIcon` 抄:异色标 `DifferentColorsMarking` 看 bit0、
炫彩标 `ColorfulMarking` 看「非异色 and bit3」——**但那只是客户端一个标记位只画一个图的取舍,
两位是可以同时置起的**(既异色又炫彩的精灵是有的),故本项目两位各自解读:`state` 记异色与否、
`glass` 记炫彩与否(非空 ⇔ 炫彩),`state=3` 且 `glass` 非空就是既异色又炫彩,前端把两样都写出来。
客户端 `MagicManualModule` 的 `FlowerSeedSort` 同样把「异色或炫彩」的花排在列表最前,
可见这就是列表侧的权威判据。等级仍不是下发字段,由星级查表算出(见 8.2 末)。

**「服务器发了没」与「不是变异」靠 `glass_info` 那一支区分**:非变异的花也带
...
——异色的权威来源仍是列表里的 `mutation_type`。炫彩的判据只能是
`battle_npc_glass_info.glass_type != GT_NULL`:同级的 `randed_battle_npc_glass` 不是炫彩标志
——炫彩的命定魔力猫 `true`、**非炫彩的命定火神也是 `true`**、非炫彩的普通铠甲虫 `false`
(`rocom-20260825-225024` / `-230448` / `-233622` 三份逐一核对),第二例即反证,拿它当标志会把
每一朵命定花种都判成炫彩。它同时用于世界首领/
传说精灵的挑战面板,故解析后要按 `npc_cfg_id` 确认是花种。落地后的实体快照(`0x014a`)里的花
实体**不带**内部精灵信息,进场景包里那朵花的 `glass_info` 是**花本身**的(恒为 `GT_NULL`),不能用。

> **`IsShinyFlower` 不是个体异色**,别看名字就用:客户端 `MagicManualModule:GetFlowerType` 里
> 它指「异色周末」活动花种(按 `activity_id` 查 `ACTIVITY_CONF.activity_type` ∈
> `ATP_SHINY_WEEKEND_PREVIEW`/`ATP_SHINY_WEEKEND_START`),与 `mutation_type` 无关;
> 同组的 `IsLimitedFlower`/`Is7StarHardFlower` 同理,都是活动类型而非个体属性。

状态四态:`0 未检测 / 1 普通 / 2 炫彩(非异色) / 3 异色`(`store.Flower*`),是否**同时**炫彩看
`glass` 列。「未检测」只在某次下发不带 `glass_info` 那一支时才出现,前端绝不能把它显示成「普通」。
...
(`store.PurgeFlowers`):该行的位置/精灵/变异结果全部作废,显示它只会误导;新的一批在下一次
`0x0375` 里到来,变异结果随之一并带来。

**② 只重投里面的精灵**:打完一场(捕捉/击败)之后——**花本身不会消失**,还留在原地、可以
再挑战一次,只是里面换成了另一只精灵(用户实测)。故这里**不撤行,只把该行打回未检测**
(`onFlowerHarvested`):种族/等级/异色炫彩作废,花的位置、血脉、有效期照旧;下一次列表
(或 `0x0376` 增量通知)会把新精灵连同它的变异带来,那之前也可由 `0x0338` 补上炫彩。判据是战斗结算 `0x132c` 里被捕捉/击败的 `npc_obj_id`——它正是**花实体**
...
`flower_seed` 表一行 = 当前开着的一朵花:`0x0375` 全量替换(列表里没有的行删掉),`0x0376`
只增改那几朵(缺席的照旧开着,别拿增量去对账)。变异列(`state`/`glass`)随描述列一起写,
但 `state=0`(未检测)**不覆盖**已有结果(`flowerUpsertSQL` 里的 `CASE`)——某次下发不带
`glass_info` 那一支时,不该把 `0x0338` 检测出的炫彩擦掉。变动后经 `GET /api/flowers` +
SSE `flowers` 推给前端(载荷已投影成 u/v,与玩家位置同一套)。

**前端**(`web/src/pages/map/useFlowers.js`):花种并入图层栏的**「地图图标」**一组(它就是张
地图图标,只是点位由流量攒出而非后端 POI 清单),**默认关**;本场景没有花种时整行不出现,
与那一组其余图层「本场景无点位就不给开关」一致。花种**没有收集模式**(异色/炫彩随列表就位,
不用逐朵去点),故排在同样没有收集开关的图层之后、带收集开关的那批(眠枭之星/不咕钟零件)之前。
标记用该花的**血脉花图**:`WorldMapNpc` 图集里 100×100 的成品图(血脉花 + 白描边 + 右上角
血脉徽章,与游戏内大地图同一张;`BigMapStatic` 里有一套同名的无边剪影,不是这张,见
rocom-parse 的 gen_icons.py 花种组)。花本体只占画布下半约一半,故标记高度另设 50px 而非
POI 的 26px,花本体才与其它 POI 图标等大;描边与投影都在素材里,前端不再自己加。
里面是**异色或炫彩**个体的描一圈金色(复用 4 节收集模式里「已确认还在」那套视觉),
**不另加标识**——是哪一种写在悬浮提示里:`{种类} Lv.55 普通` / `{种类} Lv.60 炫彩`
/ `{种类} Lv.60 异色` / 两者兼有时 `{种类} Lv.60 异色·炫彩`(最稀罕的那种,不能拿一个盖掉另一个;
等级见 8.2 末;精灵被采收后种族也未知,那时退成 `稀兽花种 Lv.55 未检测`)。
**图上有异色或炫彩花种时整行高亮**(金边 + 淡金底),省得自己在几十朵里挨个看。
